...
class Bestellungen
{
    ...
    {
        $con = self::dbcon();
        $sql = 'SELECT * FROM bestellungen WHERE status LIKE :keyword';
        $stmt = $con->prepare($sql);
        $keyword = '%' . $keyword . '%';
    ...
    {
        $con = self::dbcon();
        $sql = 'SELECT * FROM bestellungen LIMIT :limit OFFSET :offset';
        $stmt = $con->prepare($sql);
    ...
    {
        $con = self::dbcon();
        $sql = 'SELECT COUNT(*) as total FROM bestellungen';
        $stmt = $con->query($sql);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
