...
class Lieferanten {
    ...
    {
        $con = self::dbcon();
        $sql = 'SELECT * FROM lieferanten WHERE name LIKE :keyword OR kontakt LIKE :keyword';
        $stmt = $con->prepare($sql);
        $keyword = '%' . $keyword . '%';
        ...
        foreach ($results as $result) {
            $lieferanten[] = new Lieferanten($result['lieferant_id'], $result['name'], $result['kontakt'], $result['adresse']);
        }
        return $lieferanten;
    }
    ...
    {
        $con = self::dbcon();
        $sql = 'SELECT * FROM lieferanten LIMIT :limit OFFSET :offset';
        $stmt = $con->prepare($sql);
    ...
    {
        $con = self::dbcon();
        $sql = 'SELECT COUNT(*) as total FROM lieferanten';
        $stmt = $con->query($sql);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
